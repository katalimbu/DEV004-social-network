...
const db = uh(app);
const colRef = Ja(db, "post");
const q = ol(colRef, dl("createdAt", "desc"));
...
const saveUsers = (name2, email, password, nationality, Bdate, ocupation, redaRol) => Xl(Ja(db, "users"), {
  name: name2,
  email,
  password,
  nationality,
...
const deletePost = (id2) => Zl(Za(db, "post", id2)).then(() => {
  console.log("todo ok");
}).catch((error) => {
  console.log(error.message);
});
const createpost = (usuario, titulo, descripcion) => Xl(colRef, {
  usuario,
  titulo,
  descripcion,
  createdAt: df(),
...
const exitApp = () => signOut(auth).then(() => {
  console.log("the user signed out");
}).catch((err) => {
  console.log(err.message);
});
const updatePost = (id2, newPost) => Yl(Za(db, "post", id2), newPost);
const addLike = (id2, emails) => Yl(Za(db, "post", id2), { likes: emails });
const updatelike = (id2, emails) => Yl(Za(db, "post", id2), { likes: emails });
const LOCAL_ROUTES = {};
...
const showError = (code) => {
  if (code === "auth/user-not-found") {
    ...
    const wrong = document.getElementById("wrong");
    wrong.innerHTML = "Contrase\xF1a Incorrecta";
  } else if (code) {
    const wrong = document.getElementById("wrong");
...
const registerError = (code) => {
  if (code === "auth/invalid-email") {
    ...
    divErr.innerHTML = "&#10060 &#128064 el e-mail no es v\xE1lido";
  } else if (code === "auth/weak-password") {
    const divErr = document.getElementById("divParaErrores");
    divErr.innerHTML = "&#10060 &#128064 La contrase\xF1a es muy d\xE9bil";
  } else if (code === "auth/email-already-in-use") {
    const divErr = document.getElementById("divParaErrores");
    divErr.innerHTML = "&#10060 &#128064 el e-mail ya est\xE1 en uso";
  } else if (code) {
    const divErr = document.getElementById("divParaErrores");
    divErr.innerHTML = "&#10060 &#128064 Algo sali\xF3 mal";
  }
};
...
const home = () => {
  ...
  buttonRegister.textContent = "Reg\xEDstrate";
  reda.textContent = "Reda\xA9\uFE0F";
  welcome.textContent = "\xDAnete a la red de cuidadores m\xE1s grande de Latinoam\xE9rica\u2026";
...
const Login = () => {
  ...
  <div class='contenedorMadre'>
    ...
    <h1 class="redaText">\xDAnete a la red de cuidadores m\xE1s grande de Latinoam\xE9rica\u2026</h1>
    <div class="inputLogin">
      <label class='labelogin'  for="username"><b>Usuario</b></label>
      <input type="text" placeholder="Correo electr\xF3nico" name="username" id="username" required>
      <label class='labelogin'for="psw"><b>Contrase\xF1a</b></label>
      <input type="password" placeholder="Contrase\xF1a" name="psw" id="psw" required>
    </div>
    <div class="sendBtn">
 ...
 <div class= 'contLogoReg'>
  ...
  <h1 class= 'tituloReg'>  Registro de Usuario</h1>
    ...
    <div class="containerRegister">
      ...
      <label for="email" class='labelReg' ><b>Correo electr\xF3nico</b></label>
      <input type="text" placeholder="Correo electr\xF3nico" name="email" id="email" required class='inputReg'>
     
      <label for="psw" class='labelReg'><b>Contrase\xF1a</b></label>
      ...
      <label for="ocupation" class='labelReg'><b>Ocupaci\xF3n</b></label>
      <input type="text" placeholder="Ocupaci\xF3n" name="ocupation" id="ocupation" class='inputReg'>
    ...
    <div class='footerReg'<p>Reda\xA9\uFE0F</p> </div>
  </form>`;
  ...
  form.addEventListener("submit", (e) => {
    ...
    registerWithEmail(email, password).then((useCredential) => saveUsers(name2, email, password, nationality, Bdate, ocupation, redaRol)).then(() => {
      navigateTo("/home");
    }).catch((error) => {
...
const feed = () => {
  ...
  postTitle.setAttribute("placeholder", "Escribe el t\xEDtulo de tu post.");
  ...
  squareFooterF.textContent = "Reda\xA9\uFE0F";
  ...
  postContainer.addEventListener("submit", (e) => {
    ...
    const dibujar = () => {
      ...
      myPromise.then((showPost) => {
        showPost.filter((p2) => p2.likes !== void 0).forEach((postD) => {
  ...
  ${postD.descripcion}
        ...
        btnsDeletePost.forEach((btn) => {
          btn.addEventListener("click", (event) => {
            ...
            const shouldDeletePost = window.confirm("\xBFEst\xE1s seguro de que deseas eliminar este post?");
    ...
    createpost(feedUser, feedTitle, feedPost).then(() => {
      dibujar();
      postContainer.reset();
    });
  });
